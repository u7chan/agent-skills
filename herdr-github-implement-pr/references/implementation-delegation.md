...
## 4. PR 用の AI 作業メタ情報を記録する

PR 本文用に、次の役割ごとのスナップショットを親 Agent が保持する。各値はその時点で直接確認できた値だけを記録し、値が取得できなければ `—` とする。Agent 種別、起動コマンド、既定値、設定ファイルから model または effort を補完・推測しない。

| 役割 | 記録する条件 | Agent | Model | Effort |
| --- | --- | --- | --- | --- |
| オーケストレーター | 必須 | 現在の親 Agent について直接確認できた値 | 同左 | 同左 |
| 実装 | 必須 | この節で解決・起動・再利用した実装担当について直接確認できた値 | 同左 | 同左 |
| レビュー | 任意 | PR 作成前に会話または解決結果で担当 Agent が確定した場合だけ | 同左 | 同左 |
| レビューFB | 任意 | PR 作成前に会話または解決結果で担当 Agent が確定した場合だけ | 同左 | 同左 |

必須の2役割は、全列が `—` でも行を作る。任意役割は Agent 担当自体が未確定なら行を作らない。PR 作成後に解決するレビュー・レビューFB担当は、この PR 本文用スナップショットへ追加せず、既存のレビューコメント・FB 対応コメントの記録方法も変更しない。
