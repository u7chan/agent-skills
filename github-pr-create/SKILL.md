...
- formatが差分を発生させた場合、その差分を修正・stage・commitせず停止して報告する。
- 未コミット変更を意図的に作る処理は行わない。品質確認が変更を生じ得る場合は実行前状態を記録する。
...
会話で未実施のチェックを`AGENTS.md`、package設定、CI設定から解決する。存在するものだけをformat、lint/typecheck、test、buildの順で実行する。

- 実行前後の`git status --short`と`git diff`を比較する。
- formatを実行して差分が発生した場合は、生成された差分とコマンドを報告して停止する。
...
失敗した工程、コマンド、エラー概要、formatが生成した差分、既存の未コミット変更、push済みか、PR作成済みか、次に必要な対応を報告する。このSkill自身で修正や追加commitを行わない。
