...
- `github-issue-create-from-plan`、`cagent-agent-command-resolve`、`herdr-agent-delegate`を事前に読む。cagentの解決は`cagent-agent-command-resolve`、pane配置・起動・送信・Completion contractは`herdr-agent-delegate`を適用する。本スキル固有の責務とcleanup規則を優先する。
...
- Issue作成行は、`cagent low`で解決した3値を同じ`agent-command`へ固定できた場合だけ作る。同じJSONを`send_request.py --metadata-json`へ渡す。
...
2. `cagent-agent-command-resolve`のpreflightを実施する。Agent、Model、Effortは指定せずtask levelを`low`とし、doctor / dry-runで実効3値を解決する。同じ3値を明示した`agent-command`、`base-agent-type`、任意の`delegation-metadata`を得る。起動自体を解決できなければ停止する。
...
4. 入力可能確認後、`send_request.py`で依頼を送信する直前に、親が次の順で送信前処理を完了する。スナップショット取得に失敗した場合は送信せずpaneを保持して停止する。
   ...
   3. 確定済みプラン原文、対象リポジトリ、固定した行、送信指示を組み立て、`send_request.py`でEnter込みで一度だけ送信する。Issue作成担当の完全なsnapshotがある場合だけ`--metadata-json`へ渡し、標準ブロックは手書きしない。送信失敗時は読み取り専用で状態を回収し、paneを保持して停止する。
