...
description: >
  GitHub Issue や実装タスクについて、作業領域の準備、実装、検証、commit、push、PR 作成、
  Herdr 経由のレビュー、指摘対応、再チェックまでを一連で進める時に使う。
---

# GitHub Implement PR

Issue または実装指示を受け、PR 作成後のレビュー結果と対応状況まで管理する。既存スキルがある工程では、そのスキルを読んで適用する。ユーザーが作業方式を明示しない限り、現在の作業ツリーに専用ブランチを作り、停止条件に当たるまで確認を挟まず進める。
...
PR 作成後の Agent 解決、タスク内容、判定、反復、pane 管理は `references/review-loop.md` を必ず読む。Issue 確認、実装、品質確認はこのファイルの手順に従う。
...
レビュー工程の停止では作成済み PR を保持し、失敗した pane と task directory も診断用に保持する。
...
- 関連コード、設定、テストを読み、既存パターンに合わせて要求範囲だけを変更する。
- formatter、lint、test、build を確認し、変更範囲に対して最小十分な検証を行う。
- 失敗は原因を調べて修正し、再実行する。コマンドと結果を PR 本文と最終報告へ残す。
...
- `PR 作成`、`レビュー`、`FB 対応`、`再チェック` を別々の状態として示す。
- 未解決指摘、分類、試行回数、失敗理由、保持した pane / task directory、必要な判断があれば明記する。

## 品質チェック

- [ ] 既存の実装、検証、commit、PR 作成フローを維持した
- [ ] Agent 種別、既存 Agent、未指定時 Codex の分岐と作業ディレクトリが明確である
