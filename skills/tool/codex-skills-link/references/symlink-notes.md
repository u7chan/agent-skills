...
- Never delete or recreate `.codex` as a whole. Change only `.codex/skills`.
- When in doubt, inspect `.codex/skills`; replace only a confirmed symlink target and verify with `readlink -f`.
