...
3. エージェント名が `Codex` の場合は、`~/.codex/config.toml` を参照できれば `model = "..."` の値をモデル名として使う。
4. Codex 以外、または `~/.codex/config.toml` を参照できない場合は、実行環境が提供する明示的なモデル情報からモデル名を抽出する。
5. モデル名が明示されていない場合は、モデル名を推測しない。
6. 取得できた識別情報を ` / ` で結合する。
...
- Codex の場合、`~/.codex/config.toml` に `model = "gpt-1.0"` とあれば、識別子は `（Codex / gpt-1.0）` とする。
