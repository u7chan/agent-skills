...
- 別Agentへ委譲しないこと
- 実施内容、生成・変更した成果物のpathと概要、実行コマンド、検証を返すこと
- 指示から一意に決められなかった不明瞭点を返すこと。なければ「なし」とすること
- 指示にないため裁量で補完した判断と理由を返すこと。なければ「なし」とすること
...
- 成果物、不明瞭点、裁量補完、再試行回数と理由、steps概数を回収できる。
- 非公開チェック項目を証拠付きで採点できる。

durationはworking確認時刻から最終状態確認時刻までを記録する。stepsとretriesはAgent報告を回収出力と成果物に照らし、評価プロトコルの定義で親が確定する。不明瞭点と裁量補完は混同せず別々に重複除外する。正常に回収しても必要項目が欠ける、または採点不能なら成功扱いにしない。
