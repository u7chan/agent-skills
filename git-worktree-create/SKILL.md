...
- `herdr-github-implement-pr` などの上位スキルが、PR 実装用の作業領域を用意する時
...
- worktree の作成に `git worktree add` を使わない。必ず `herdr worktree create --path <絶対パス> --branch <ブランチ名>` を使う。
...
- 作成元の解決は Herdr に任せ、`git worktree add` や `git switch` で代替しない。
...
worktree-path: /home/user/work/agent-skills-worktrees/feature-issue-91-git-worktree-create-skill
```
...
herdr worktree create --path <worktree-pathの絶対パス> --branch <branch>
```

`--path` には必ず絶対パスを渡す。作成済みブランチや worktree を開くだけの場合も、作成処理へ混ぜず停止して再利用可否を確認する。
...
- [ ] worktree 作成前に `git fetch origin` を実行している
...
- [ ] `git worktree add` ではなく `herdr worktree create --path <絶対パス> --branch <ブランチ名>` を使っている
