...
コーディングエージェントが並列で安全に作業できるように、現在の作業ツリーを切り替えず、独立した `git worktree` と作業ブランチを作成するスキル。ブランチ名は `git-branch-create` の命名ルールを使い、実際の作業領域作成はこのスキルで行う。
...
- `herdr-github-pr-orchestrate`などの上位スキルが、PR実装用の作業領域を用意する時
